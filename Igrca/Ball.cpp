...
Ball::Ball(float radius) : dragged(false), mass(radius * 20.f), circleShape(sf::CircleShape(radius)) {   //inicalizacija zoge
...
void Ball::update(float deltatime) {  //zelo pomembna funkcija za zogo, sam updejta njeno pozicijo in glede ce se odbije ob kake stene(ne pravokotnikov temvec zunanjih sten windowa(igra bila najprej implementirana brez pravkkotnikov))
...
void Ball::draw(sf::RenderWindow& window) {    //narisi zogo
	window.draw(circleShape);
}

void Ball::setPosition(float x, float y) {   //nastavi pozicijo
